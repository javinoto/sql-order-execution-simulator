...
export const SQLConsole: React.FC<SQLConsoleProps> = ({ step, onNext, onPrev, onReset, canNext, canPrev }) => {
  ...
  const lines = [
    { id: 'select', html: <><span className="text-fuchsia-400">SELECT</span> Name, Product, Amount</>, activeStep: Step.SELECT },
  ...
  return (
    <div className="w-full flex flex-col gap-3"> {/* Gap reducido entre visor y controles */}
      ...
      <div className="flex items-center gap-3 bg-[#0B1121] p-2 pr-4 rounded-xl border border-white/10 shadow-lg">
           <button onClick={onReset} className="p-2.5 rounded-lg text-slate-500 hover:text-white hover:bg-white/5 transition-all" title="Reset">
              <RotateCcw size={16} />
           </button>
           ...
           <div className="flex items-center gap-1 flex-1 justify-center">
              <button onClick={onPrev} disabled={!canPrev} className="p-2.5 hover:bg-white/5 rounded-lg text-white disabled:opacity-20 transition-all">
                  <ChevronLeft size={20} />
              </button>
              ...
              <button 
                ...
                className={`
                   mx-2 w-12 h-9 rounded-md flex items-center justify-center transition-all duration-300
                   ${isPlaying 
              ...
              <button onClick={onNext} disabled={!canNext} className="p-2.5 hover:bg-white/5 rounded-lg text-white disabled:opacity-20 transition-all">
                  <ChevronRight size={20} />
              </button>
           </div>

           <div className="text-[9px] font-mono tracking-widest text-slate-600 uppercase">
              {isPlaying ? <span className="text-cyan-500 animate-pulse">Running</span> : 'Paused'}
           </div>
      </div>
